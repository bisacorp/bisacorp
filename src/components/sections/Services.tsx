...
import { Code, Smartphone, BrainCircuit, Palette, CheckCircle2 } from "lucide-react";
...
export function Services() {
  ...
  return (
    <section id="services" className="py-24 bg-background/50 transition-colors duration-300">
      <div className="container mx-auto px-6">
        <div className="max-w-3xl mb-16">
          <h2 className="text-sm font-bold text-accent uppercase tracking-widest mb-4">{t.services.badge}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {services.map((service, idx) => (
            <div key={idx} className="group perspective h-[400px] cursor-pointer">
              <div className="relative w-full h-full transition-all duration-700 preserve-3d group-hover:[transform:rotateY(-180deg)]">
                ...
                <Card className="absolute inset-0 backface-hidden border border-border shadow-lg bg-card flex flex-col">
                  ...
                  <CardContent className="px-8 pb-8 mt-auto">
                    <p className="text-xs font-bold text-accent uppercase tracking-wider">{t.services.hoverHint}</p>
                  </CardContent>
                </Card>
                ...
                <Card className="absolute inset-0 backface-hidden rotate-y-180 border-none shadow-lg bg-primary dark:bg-zinc-900 text-white dark:text-foreground flex flex-col justify-center overflow-hidden">
                  ...
                  <CardHeader className="px-8 relative z-10">
                    <CardTitle className="text-xl font-bold text-accent mb-4">{t.services.keyFeatures}</CardTitle>
                  </CardHeader>
